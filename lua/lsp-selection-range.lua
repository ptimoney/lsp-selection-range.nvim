...
function M.setup(user_config)
  ...
  vim.api.nvim_create_autocmd("LspAttach", {
    group = vim.api.nvim_create_augroup("lsp_selection_range_keymaps", { clear = true }),
    callback = function(args)
      ...
      if config.keymaps.expand then
        vim.keymap.set(config.modes, config.keymaps.expand, function()
          selection_range(false)
        end, vim.tbl_extend("force", opts, { desc = "LSP: Expand selection" }))
      end

      if config.keymaps.shrink then
        vim.keymap.set(config.modes, config.keymaps.shrink, function()
          selection_range(true)
        end, vim.tbl_extend("force", opts, { desc = "LSP: Shrink selection" }))
      end
    end,
  })
end
